...
\chapter{Аеродинамический расчёт}

\section{Расчёт подъёмной силы $c_y$}

\subsection{Некоторые пояснения}
...
\subsection{Подготовительные расчёты}
...
\subsection{Расчёт аэродинамических характеристик изолированных элементов}
Для каждого числа Маха $M[i]$:
\begin{align*}
c_{y\alpha_{\text{кр}}}[i] &= \frac{\texttt{get\_c\_y\_alpha\_IsP}(M[i], \lambda, \bar{c}, \chi_{0.5}, \zeta)}{57.3} \\
c_{y\alpha_{\text{оп}}}[i] &= \frac{\texttt{get\_c\_y\_alpha\_IsP}(M[i], \lambda_{\text{оп}}, \bar{c}, \chi_{0.5}, \zeta)}{57.3} \\
c_{y\alpha_{\text{изкор}}}[i] &= \frac{\texttt{get\_c\_y\_alpha\_NosCil\_Ell}(M[i], \lambda_{\text{нос}}, \lambda_{\text{цил}})}{57.3}
\end{align*}

\subsection{Учёт пограничного слоя}

\begin{align*}
\bar{\delta}_{\text{звз}}[i] &= \frac{0.093 L_1 (1 + 0.4M[i] + 0.147M[i]^2 - 0.006M[i]^3)}{(V[i]L_1/\nu)^{0.2}D} \\
x_{\text{пс кр}}[i] &= \left(1 - \frac{2\bar{D}^2 \bar{\delta}_{\text{звз}}[i]}{1 - \bar{D}^2}\right) \left(1 - \frac{\bar{D}(\eta_k - 1)\bar{\delta}_{\text{звз}}[i]}{(1 - \bar{D})(\eta_k + 1)}\right)
\end{align*}

\subsection{Расчёт коэффициентов интерференции}

Для $M[i] \geq 1$:
\begin{align*}
\pi M D_{\text{кв}}[i] &= \frac{\pi}{2} D \sqrt{M[i]^2 - 1} \\
\bar{l}_{xv_{\text{кр}}}[i] &= \frac{L_{xv_k}}{\pi M D_{\text{кв}}[i]} \\
\bar{b}_{\text{кр}}[i] &= \frac{b_{\text{кр}}}{\frac{\pi}{2} D \sqrt{M[i]^2 - 1}} \\
K_{\alpha \alpha_{\text{кр}}}[i] &= \begin{cases}
K_{\alpha \alpha_{\text{теор}}} \kappa_{\text{нос}} x_{\text{пс кр}}[i] \kappa_M, & \text{если } \bar{l}_{xv_{\text{кр}}}[i] > \pi M D_{\text{кв}}[i] \\
(k_{\alpha \alpha_{\text{теор}}} + (K_{\alpha \alpha_{\text{теор}}} - k_{\alpha \alpha_{\text{теор}}}) F_{L_{xv_{\text{кр}}}}[i]) x_{\text{пс кр}}[i] \kappa_{\text{нос}} \kappa_M, & \text{иначе}
\end{cases}
\end{align*}

\subsection{Расчёт скоса потока}

\begin{align*}
\bar{z}[i] &= \texttt{get\_bar\_z\_v}(M[i], \lambda, \chi_{0.5}, \zeta) \\
z_v[i] &= 0.5(D + \bar{z}[i](l_{\text{размах зад}} - D)) \\
i_v[i] &= \texttt{get\_i\_v}(\zeta, D, l_{\text{размах зад}}, y_v, z_v[i]) \\
\psi_\epsilon[i] &= \texttt{get\_psi\_eps}(M[i], \alpha_p, \varphi_\alpha, \psi_I, \psi_{II}, z_v[i], y_v, L_{vI_{bII}}, D, l_{1c_{II}}, \zeta_{II}, b_{b_{II}}, \chi_{0_{II}}) \\
\varepsilon_{\alpha_{\text{ср}}}[i] &= \frac{57.3}{2\pi} \cdot \frac{i_v[i]}{\bar{z}[i]} \cdot \frac{l_{\text{размах оп}}}{l_{\text{размах конс}}} \cdot \frac{c_{y\alpha_{\text{оп}}}[i]}{\lambda_{\text{оп}}} \cdot \frac{k_{aa_{\text{оп}}}[i]}{K_{aa_{\text{кр}}}[i]} \cdot \psi_\epsilon[i]
\end{align*}

\subsection{Учёт торможения потока}

\begin{align*}
\kappa_{q_{\text{нос}}}[i] &= \texttt{get\_kappa\_q\_Nos\_Con}(M[i], \lambda_{\text{нос}}) \\
\kappa_{q_{\text{исп}}}[i] &= \texttt{get\_kappa\_q\_IsP}(M[i], L_A, b_{\text{оп}})
\end{align*}

\subsection{Формирование итоговых аэродинамических характеристик}
\begin{align*}
S &= S_{\text{оп}} + S_{\text{конс}} + S_f \\
\bar{s}_f &= \frac{S_f}{S}, \quad \bar{s}_1 = \frac{S_{\text{оп}}}{S}, \quad \bar{s}_2 = \frac{S_{\text{конс}}}{S} \\
c_{y\alpha_1}[i] &= c_{y\alpha_f}[i] \bar{s}_f + c_{y\alpha_1}[i] \bar{s}_1 \kappa_{t1}[i] + c_{y\alpha_2}[i] \bar{s}_2 \kappa_{t2}[i] \\
c_{y1}[i] &= c_{y\alpha_1}[i] \cdot \alpha[2000]
\end{align*}
